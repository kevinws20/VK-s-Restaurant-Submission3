...
const restoDetail = (resto) => `
  <div class="detail">
    <div class="img-container">
        <img class="detail-img" alt="${resto.name}" src="${CONFIG.BASE_IMAGE_URL + resto.pictureId}" crossorigin="anonymous"/>
    </div>

    <ul class="detail-info">
      <li>
        <i title="restaurant" class="fas fa-store-alt icon-primary"></i>
        <h1 class="detail-name-address-rating">${resto.name}</h1>
      </li>
      ...
      <li>
        <p class="detail-name-address-rating">⭐️ ${resto.rating}</p>
      </li>

      <li><p class="detail-desc">${resto.description}</p></li>

      <li>${resto.categories.map(
        (category) => `
